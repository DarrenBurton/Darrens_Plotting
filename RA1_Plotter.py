...
class List_Files(object):
  ...
  def File_Chooser(self, **karg):
      self.Default_Colours = [1,4,7,3,5,46,9,10]
      self.Hist_Colour = []
      self.Scale_Factor = 0
      Pick_File = []
      Pick_MC = []
      ...
      if Progress == None:
        ...
        if karg["Type"] == "Full":
          ...
          if Pick_MC == "2":
              ...
              self.Pick_LMPoint = raw_input("Choose LMPoint File... ")
              self.Pick_PointNumber = raw_input("Choose Point Number...")
              Progress = Menu_Opt.Wrong_Selection(self.Pick_LMPoint, str(self.Root_File_Num), lambda: self.File_Chooser(Type = self.plot_type, Data = self.is_data, addition = self.is_funky))    
              self.Check_pass(self.Pick_LMPoint, "LM %s" %self.Pick_PointNumber,6)
...
class PlotSelector(object):
    ...
    def Folder_Action(self):  
        ...
        if self.action == "A" :
            ...
            all_statement = True
            if self.options["Funky"] == True: 
              length_passer = len(self.dir_passer)
              self.subfolder_choice.append(self.subfolder[0])
