...
def GetAllhistos(inFile):
  temp = Root.TFile.Open(inFile).Get("AlphaT55")
...
def GethistFromFolder(DataSetName,folder,hist,col,norm,Legend):
    a = Root.TFile.Open(DataSetName).Get("AlphaT55") #open the file
...
histList = GetAllhistos("../data.root")#["HT_aftar_AlphaT_all"]#"JetMultiplicityAfterAlphaT_all",
#JetThreshList = ["","37","43"]
#for num in JetThreshList:
for dir in GetAllSubFiles("../data.root"):
   if dir == "susyTree" : continue
   for hist in histList:
      outdir = outdirTmp
      print dir,hist
      Data =  GethistFromFolder("../data.root",dir,hist,1,0,"Data")
     ...
     # MCCentral.Draw("histsame")
      ...
      outdir = outdir
      ensure_dir(outdir)
      c1.SaveAs(outdir+"/"+hist+"_"+dir+".png")
      leg.Clear()
      for a in closeList:
        a.Close()
